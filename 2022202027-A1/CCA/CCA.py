...
with open("./inputs/cca.csv", 'r') as file:
  heading = next(file)
  csvreader = csv.reader(file)
  ofile = open('./output/cca.txt','r')
  out = ofile.read().split('\n')
  i=1
  for col in csvreader:
    a = CCA(int(col[0]),int(col[1]),int(col[2]),int(col[3]),[int(col[4]),int(col[5])])
    temp = a.enc(col[6],int(col[7]))
    print(temp,out[i])
    if(temp == out[i]):
        print("True")
    print(a.dec(temp)==col[6])
    i+=1